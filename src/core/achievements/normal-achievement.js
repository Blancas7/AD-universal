...
class AchievementState extends GameMechanicState {
  ...
  unlock(auto) {
    ...
    if (this.id === 55 && !PlayerProgress.realityUnlocked() && !PlayerProgress.rewindUnlocked()) {
...
export const Achievements = {
   ...
   */
  get preRewind() {
    return Achievements.all.filter(ach => ach.isPreRewind);
  },
  ...
  get preRewindRows() {
    const count = Achievements.preRewind.map(a => a.row).nMax();
    return Achievements.rows(1, count);
  },
