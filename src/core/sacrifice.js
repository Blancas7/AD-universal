...
export class Sacrifice {
  ...
  static get isVisible() {
    return Achievement(18).isUnlocked || PlayerProgress.realityUnlocked() || PlayerProgress.rewindUnlocked();
  }
...
export function sacrificeReset() {
  ...
  if ((!player.break || (!InfinityChallenge.isRunning && NormalChallenge.isRunning)) &&
    Currency.antimatter.gt(DC.D_MAX_NUMBER)) return false;
  if (
    NormalChallenge(8).isRunning &&
    (Sacrifice.totalBoost.gte(DC.D_MAX_NUMBER))
  ) {
    return false;
  }
