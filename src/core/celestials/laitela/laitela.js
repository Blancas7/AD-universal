...
export const Laitela = {
  ...
  get matterExtraPurchaseFactor() {
    return (1 + 0.5 * Math.pow(Decimal.pLog10(Currency.darkMatter.max).div(50).toNumber(), 0.4) *
  ...
  get entropyGainPerSecond() {
    return Math.clamp(Math.pow(Currency.antimatter.value.add(1).log10().div(1e11).toNumber(), 2), 0, 100) / 200;
  },
  ...
  maxAllDMDimensions(maxTier) {
    ...
    for (const upgrade of upgradeInfo) {
      const purchases = Decimal.clamp(Decimal.floor(darkMatter.times(0.02).div(upgrade[0]).log(upgrade[1])), 0, upgrade[2]);
  ...
  reset() {
    ...
    this.celestial.entropy = 0;
    this.celestial.thisCompletion = 3600;
