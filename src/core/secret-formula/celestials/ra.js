export const ra = {
  pets: {
    teresa: {
      ...
      rawMemoryChunksPerSecond: () => 4 * Math.pow(Currency.eternityPoints.value.pLog10().div(1e4).toNumber(), 3),
    ...
    enslaved: {
      ...
      rawMemoryChunksPerSecond: () => 4 * Math.pow(Currency.timeShards.value.pLog10().div(3e5).toNumber(), 2),
    ...
    v: {
      ...
      rawMemoryChunksPerSecond: () => 4 * Math.pow(Currency.infinityPower.value.pLog10().div(1e7).toNumber(), 1.5),
  ...
  unlocks: {
    ...
    teresaXP: {
      ...
      effect: () => 1 + Math.pow(Currency.realityMachines.value.pLog10().div(100).toNumber(), 0.5),
      pet: "teresa",
      level: 5,
      displayIcon: `Ϟ`
    },
    ...
    enslavedXP: {
      id: 16,
      reward: "All Memory Chunks produce more Memories based on time played in this Universe",
      effect: () => 1 + Math.log10(player.thisRewind.time) / 200,
      pet: "enslaved",
      level: 5,
