...
export const alchemyResources = {
  ...
  "momentum": {
    ...
    formatEffect: value => `All Dimensions ${formatPow(Ra.momentumValue, 4, 4)}, increasing by
      ${format(0.005 * Achievement(175).effectOrDefault(1) * player.rewind.celestialMultiplier, 3, 3)}
  ...
  "uncountability": {
    ...
    formatEffect: value => `Generate ${format(value * player.rewind.celestialMultiplier, 2, 2)} Realities and Perk Points per second`,
    reagents: [
      {
        resource: ALCHEMY_RESOURCE.INFINITY,
        amount: 20
