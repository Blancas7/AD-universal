...
export const celestialNavigation = {
  ...
  "effarig-reality-unlock": {
    ...
    complete: () => (EffarigUnlock.run.isUnlocked
      ? 1 : Math.clampMax(0.999, Decimal.pLog10(Currency.relicShards.value).div(Math.log10(EffarigUnlock.run.cost)).toNumber())),
    node: {
      clickAction: () => Tab.celestials.effarig.show(true),
      completeClass: "c-celestial-nav__effarig",
      incompleteClass: "c-celestial-nav__test-incomplete",
  ...
  "effarig-infinity": {
    ...
    complete: () => {
      ...
      return Currency.antimatter.value.pLog10().div(DC.D_MAX_NUMBER.log10()).toNumber();
    },
    node: {
      clickAction: () => Tab.celestials.effarig.show(true),
      completeClass: "c-celestial-nav__effarig",
  ...
  "effarig-eternity": {
    ...
    complete: () => {
      ...
      return Currency.infinityPoints.value.pLog10().div(DC.D_MAX_NUMBER.log10()).toNumber();
    },
    node: {
      clickAction: () => Tab.celestials.effarig.show(true),
      completeClass: "c-celestial-nav__effarig",
  ...
  "laitela-unlock": {
    ...
    complete: () => {
      ...
      return Math.clampMax(0.5, Currency.realityMachines.value.pLog10().div(MachineHandler.baseRMCap.exponent).toNumber());
    },
    drawOrder: -1,
    node: {
  ...
  "pelle-unlock": {
    ...
    complete: () => {
      ...
      let laitelaProgress = Laitela.isRunning ? Math.min(Currency.eternityPoints.value.log10().div(4000).toNumber(), 0.99) : 0;
