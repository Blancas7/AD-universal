...
export const normalAchievements = [
  ...
  },
  {
    ...
    checkEvent: [GAME_EVENT.REALITY_RESET_AFTER, GAME_EVENT.REWIND_RESET_AFTER]
  },
  {
    id: 23,
  ...
  },
  {
    ...
    checkEvent: [GAME_EVENT.BIG_CRUNCH_AFTER, GAME_EVENT.REALITY_RESET_AFTER, GAME_EVENT.REWIND_RESET_AFTER],
    get reward() {
      return `All Antimatter Dimensions are stronger in the first ${formatInt(3)} minutes of Infinities,
  ...
  },
  {
    ...
    get description() {
      return `Get all Antimatter Dimension multipliers over ${formatX(DC.D_MAX_NUMBER, 1)}.`;
    },
    checkRequirement: () => AntimatterDimensions.all.every(x => x.multiplier.gte(DC.D_MAX_NUMBER)),
    checkEvent: GAME_EVENT.GAME_TICK_AFTER,
  ...
  },
  {
    ...
    get description() {
      return `Get a ${formatX(DC.D_MAX_NUMBER, 1, 0)} multiplier in a single Dimensional Sacrifice.`;
    },
    checkRequirement: () => Sacrifice.nextBoost.gte(DC.D_MAX_NUMBER),
  ...
  },
  {
    id: 95,
    name: "Is this safe?",
    get description() { return `Gain ${format(DC.D_MAX_NUMBER, 1, 0)} Replicanti in ${formatInt(1)} hour.`; },
    get reward() { return `You keep your Replicanti and ${formatInt(1)} Replicanti Galaxy on Infinity.`; },
    checkRequirement: () =>
      (Replicanti.amount.eq(DC.D_MAX_NUMBER) || player.replicanti.galaxies > 0) &&
  ...
  },
  {
    ...
    get description() {
      ...
      ${format(DC.D_MAX_NUMBER, 1, 0)} times higher Infinity Points than the previous one.`;
    },
    checkRequirement: () => {
      ...
      for (let i = 0; i < infinities.length - 1; i++) {
        if (infinities[i].lt(infinities[i + 1].times(DC.D_MAX_NUMBER))) return false;
      }
      return true;
    },
    checkEvent: GAME_EVENT.BIG_CRUNCH_AFTER,
  ...
  },
  {
    ...
    checkEvent: GAME_EVENT.GAME_TICK_AFTER,
    get reward() {
      return `Replicanti Galaxies divide your Replicanti by ${format(DC.D_MAX_NUMBER, 1, 0)}
  ...
  },
  {
    ...
    get description() { return `Reach ${format(DC.D_MAX_NUMBER, 1, 0)} Eternity Points.`; },
    checkRequirement: () => Currency.eternityPoints.gte(DC.D_MAX_NUMBER),
    checkEvent: GAME_EVENT.GAME_TICK_AFTER
  },
  {
    id: 128,
  ...
  },
  {
    ...
    get description() {
      ...
      ${format(DC.D_MAX_NUMBER, 1, 0)} times higher Eternity Points than the previous one.`;
    },
    checkRequirement: () => {
      ...
      for (let i = 0; i < eternities.length - 1; i++) {
        if (eternities[i].lt(eternities[i + 1].times(DC.D_MAX_NUMBER))) return false;
      }
      return true;
    },
    checkEvent: GAME_EVENT.ETERNITY_RESET_AFTER,
  ...
  },
  {
    ...
    get description() { return `Get ${format(DC.D_MAX_NUMBER, 1)} Infinities.`; },
    checkRequirement: () => Currency.infinitiesTotal.gte(DC.D_MAX_NUMBER),
    checkEvent: GAME_EVENT.GAME_TICK_AFTER,
  ...
  },
  {
    ...
    get description() { return `Reach ${format(DC.D_MAX_NUMBER, 1, 0)} Reality Machines.`; },
    checkRequirement: () => Currency.realityMachines.gte(DC.D_MAX_NUMBER),
    checkEvent: GAME_EVENT.GAME_TICK_AFTER,
  ...
  },
  {
    ...
    get description() {
      return `Reality for ${format(DC.D_MAX_NUMBER, 1)} Reality Machines without having
      any Charged Infinity Upgrades, having any equipped Glyphs, or buying any Triad Studies.`;
    },
    checkRequirement: () => MachineHandler.gainedRealityMachines.gte(DC.D_MAX_NUMBER) &&
