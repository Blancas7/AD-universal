...
export function getDilationGainPerSecond() {
  ...
  dtRate = dtRate.times(
    Decimal.clampMin(Decimal.log10(Replicanti.amount.max(1)).times(getAdjustedGlyphEffect("replicationdtgain")), 1));
...
export function getDilationTimeEstimate(goal) {
  ...
  if (PelleRifts.paradox.isActive) {
    ...
    return TimeSpan.fromSeconds(currNetRate.div(goalNetRate).ln().div(drain).toNumber()).toTimeEstimate();
  }
...
export function dilatedValueOf(value) {
  ...
  return Decimal.pow10(Decimal.pow(Decimal.abs(log10), dilationPenalty).times(Decimal.sign(log10)));
}
