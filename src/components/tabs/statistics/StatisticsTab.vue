...
export default {
  ...
  methods: {
    update() {
      ...
      const bestRewind = records.bestRewind;
...
<template>
  <div class="c-stats-tab">
    ...
    <div
      v-if="rewind.isUnlocked"
      class="c-stats-tab-subheader c-stats-tab-general"
    >
      <div class="c-stats-tab-title c-stats-tab-rewind">
        Rewind
      </div>
      <div>You have {{ quantifyInt("Rewind", rewind.count) }}.</div>
      <div>Your fastest game-time Rewind was {{ rewind.best.toStringShort() }}.</div>
      ...
      <div>
        Your best Unity Points per minute is {{ format(rewind.bestRate, 2, 2) }}.
      </div>
      <br>
    </div>
  </div>
