  ...
  methods: {
    ...
    update() {
      ...
      const setProgress = (current, goal, tooltip) => {
        this.fill = Math.clampMax(current.pLog10().div(Decimal.log10(goal)).toNumber(), 1);
      ...
      if (inSpecialRun) {
        ...
        if (PelleRifts.recursion.milestones[2].canBeApplied || GalaxyGenerator.spentGalaxies > 0) {
          ...
          if (player.break) {
            ...
            setProgress(Currency.antimatter.value, DC.D_MAX_NUMBER, "Percentage to Infinity");
          }
        } else {
          setProgress(Currency.antimatter.value, DC.D_MAX_NUMBER, "Percentage to first Strike");
        }
